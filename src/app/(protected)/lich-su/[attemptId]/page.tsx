...
// Câu có nhiều gap? (backend trả kèm gaps + gapResult cho câu FILL nhiều ô)
        ...
        {data.questions.map((q: any) => {
          ...
          return (
            <div key={q.id} className={`card !py-4 border-l-4 ${q.isCorrect === true ? "border-l-green-500" : q.isCorrect === false ? "border-l-red-500" : "border-l-silver"}`}>
              ...
              {/* Câu NHIỀU GAP: GapPlayer render nội dung + tô đúng/sai từng ô (read-only) */}
              {gapMap ? (
                <div className="mb-3 overflow-x-auto text-sm text-[#1a1a2e]">
                  <GapPlayer
                    content={q.content}
                    gaps={gapMap}
                    answers={(q.studentAnswer && typeof q.studentAnswer === "object") ? q.studentAnswer : {}}
                    result={q.gapResult ? { detail: q.gapResult.detail } : null}
                    onChange={() => {}}
                  />
                </div>
              ) : (
              ...
              {!gapMap && q.type !== "MULTIPLE_CHOICE" && q.type !== "ESSAY" && (
